...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import Deldir_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("Deldir_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "Deldir_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"Deldir", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "Deldir_jll", "src", "Deldir_jll.jl"))
        @info("Deldir_ll dev'ed out to /depot/dev/Deldir_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("0cb025a2-29c7-561e-a277-a82c5b779c13"))
platforms = [Pkg.Artifacts.unpack_platform(e, "Deldir", artifacts_toml) for e in artifacts["Deldir"]]
...
if best_platform === nothing
    @debug("Unable to load Deldir; unsupported platform $(triplet(platform_key_abi()))")
else
